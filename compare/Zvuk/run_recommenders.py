...
save_outputs = False
log_dir = './compare/Zvuk/logs_recommenders'
...
data_fraction = 0.25  # Taking this percent of last interactions as the data
interactions = pd.read_parquet("./zvuk/zvuk-interactions.parquet")
interactions = interactions.rename(columns={"datetime": "timestamp", "track_id": "item_id", "play_duration": "rating"})
print(interactions.shape)
interactions = interactions.loc[interactions["rating"] >= 30., ["user_id", "item_id", "timestamp"]]  # Only positive feedbacks
print(interactions.shape)
interactions["timestamp"] = pd.to_datetime(interactions["timestamp"]).astype(np.int64) * 10**6 + np.arange(0, interactions.shape[0])
...
def get_predictions_for_test_users(model, dataset, maxlen):
    ...
    with device:
        ...
        for user in tqdm(dataset.user_test.keys(), desc="Predicting", ncols=70):
            ...
            probabilities = softmax(model.predict(inputs)[0].numpy())
